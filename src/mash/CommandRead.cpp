...
int CommandRead::run() const
{
    ...
    const Sketch::Reference & refRef = sketchRef.getReference(0);
    const HashList & hashRef = refRef.hashesSorted;

    for(uint64_t i = 0; i < hashRef.size(); i++){
        cout << hashRef.at(i).hash64 << endl;
    }
