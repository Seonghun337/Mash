...
int Sketch::writeOneToCapnp(const char * file) const{
    ...
    capnp::List<capnp::MinHash::ReferenceList::Reference>::Builder referencesBuilder = referenceListBuilder.initReferences(references.size()+1);
    
    cerr << "init complete\n";

    for ( uint64_t _i = 1; _i < references.size()+1; _i++ )
    {
        int i = _i-1;
    ...
    int hash_size = parameters.minHashesPerWindow;
    // std::vector<int> j_arr;
    int j_arr[hash_size];
    fill_n(j_arr,hash_size,0);

    // capnp::MallocMessageBuilder _message;
    // capnp::MinHash::Builder _builder = message.initRoot<capnp::MinHash>();
    // capnp::MinHash::ReferenceList::Builder resultRefListBuilder = (parameters.seed == 42 ? _builder.initReferenceListOld() : _builder.initReferenceList());
    // capnp::List<capnp::MinHash::ReferenceList::Reference>::Builder resultRefBuilder = resultRefListBuilder.initReferences(1);
    
    // capnp::MinHash::ReferenceList::Reference::Builder _referenceBuilder = resultRefBuilder[0];
    // _referenceBuilder.setName(references[0].name);
    // _referenceBuilder.setComment(references[0].comment);
    // _referenceBuilder.setLength64(references[0].length);
    capnp::List<uint64_t>::Builder hashes64Builder = referencesBuilder[0].initHashes64(hash_size);

    hash_u prev;
    for (uint64_t i=0; i< hash_size; i++){
        
        hash_u min = references[0].hashesSorted.at(hash_size-1);
        int min_j = 0;
        for (uint64_t j=0; j<references.size(); j++){
            while(references[j].hashesSorted.at(j_arr[j]).hash64 == prev.hash64)
                j_arr[j]++;

            if (references[j].hashesSorted.at(j_arr[j]).hash64 < min.hash64){
                min = references[j].hashesSorted.at(j_arr[j]);
                min_j = j;
            }
        }
        j_arr[min_j]++;
        //새로운 해시
        hashes64Builder.set(i, min.hash64);
        prev.hash64 = min.hash64;
    }
...
Sketch::SketchOutput * loadCapnp(Sketch::SketchInput * input)
{
    ...
    for ( uint64_t i = 0; i < lociReader.size(); i++ )
    {
        ...
        output->positionHashesByReference[locusReader.getSequence()].push_back(
        Sketch::PositionHash(locusReader.getPosition(), locusReader.getHash64()));
    }
