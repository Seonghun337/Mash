...
: Command()
{
    ...
    addOption("combine", Option(Option::Boolean, "e", "Sketch", "Combine sketches to one sketch by extracting min hash", ""));
    useSketchOptions();
}
...
int CommandSketch::run() const
{
    ...
    if ( getOption("combine").active )
	{
        cerr << "start\n";
        cerr << prefix.c_str();
		sketch.writeOneToCapnp(prefix.c_str());
	}
    else{
